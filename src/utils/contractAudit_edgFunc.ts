 ...
 * 功能：接收合同图片，调用 AI 接口进行合同识别和审计
 */

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

const CONTRACT_AUDIT_PROMPT = `你是一个专业的合同审计 AI。请仔细分析上传的合同图片，进行以下审计：
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('审计请求数据:', { imageData: body.imageData ? '[图片数据]' : '无', nodeText: body.nodeText });
    ...
    if (!openRouterApiKey) {
      ...
      const mockResult = {
        summary: '这是一份装修合同样例，实际使用时请配置 AI API Key',
        hasIssue: true,
        overallScore: 7,
        issues: [
          {
    ...
    try {
      await supabase
        ...
        .insert({
          ...
          node_text: nodeText || '',
          image_data: imageData.substring(0, 1000), // 只保存缩略数据
